...
export default class EASIAppUtils {
   ...
   */
  static toAppWeb({scheme = 'au.com.easi.courier', title, url, show}: IToAppWebOption) {
  ...
  // 跳App原生页面
  static toAppPage(path: string, scheme = 'au.com.easi.customer') {
    window.location.href = `${scheme}://${path}`;
  }

  // 判断App版本号,目前只支持用户端App
  static compareVersion(targetVersion: string, ua = navigator.userAgent.toLowerCase()) {
    try {
      let system = ua
  ...
  // 判断AppUserAgent是否合法
  static compareUserAgent(clientTypeArray: IClientType[] = [], ua = navigator.userAgent.toLowerCase()) {
